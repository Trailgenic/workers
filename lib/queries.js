...
import protocolMatchedHikes from "../datasets/terrain_intelligence/tg_protocol_matched_hikes_v2.json" with { type: "json" };
...
import hikingWorldModel from "../datasets/hiking/tg_hikeworldmodel_v3_1.json" with { type: "json" };
...
const resultSet = ({ dataset, records, filters = {}, limit, recordKey = "records" }) => {
  ...
  return {
    ...
    ...(dataset.status ? { status: dataset.status } : {}),
    ...(dataset.last_updated ? { last_updated: dataset.last_updated } : {}),
    ...(dataset.canonical_alignment ? { canonical_alignment: dataset.canonical_alignment } : {}),
    ...(dataset.evidence_boundary ? { evidence_boundary: dataset.evidence_boundary } : {}),
...
export const getProtocols = (args = {}) => {
  ...
  return {
    ...resultSet({ dataset: protocols, records, filters: { protocol_id: args.protocol_id }, recordKey: "protocols" }),
    scope: protocols.scope,
    governor: protocols.governor
  };
};
...
export const getPhysiologyAdaptation = (args = {}) => {
  if (!args.module) {
    return {
      ...
      note: "Pass a module slug to return module records. Without a module, this tool returns the public physiology module catalog, including qualification status.",
      modules: PHYSIOLOGY_MODULES.map((module) => ({
        slug: module.slug,
        title: module.title,
        endpoint: `/datasets/physiology-adaptation/${module.slug}`,
        status: MODULE_DATA[module.slug]?.status ?? "unknown",
        canonical_alignment: MODULE_DATA[module.slug]?.canonical_alignment
      }))
    };
  }
...
export const getHikingWorldModel = () => hikingWorldModel;
...
export const getProtocolMatchedHikes = (args = {}) => {
  const records = arrayFrom(protocolMatchedHikes, "hikes").filter((record) =>
    equalsIfPresent(record, "region", args.region) &&
    equalsIfPresent(record, "protocol_level", args.protocol_level) &&
    equalsIfPresent(record, "evidence_tier", args.evidence_tier) &&
    equalsIfPresent(record, "recovery_cost", args.recovery_cost)
  );

  return resultSet({
    dataset: protocolMatchedHikes,
    records,
    filters: {
      region: args.region,
    ...
    limit: boundedLimit(args.limit, 22),
    recordKey: "hikes"
  });
};
...
export const TOOL_HANDLERS = new Map([
  ...
  ["tg.hiking.worldModel.get", getHikingWorldModel],
  ...
  ["tg.terrain.protocolMatchedHikes.get", getProtocolMatchedHikes],
...
export const DATASET_JSON_BY_SOURCE_PATH = new Map([
  ...
  ["datasets/hiking/tg_hikeworldmodel_v3_1.json", hikingWorldModel],
  ...
  ["datasets/terrain_intelligence/tg_protocol_matched_hikes_v2.json", protocolMatchedHikes],
