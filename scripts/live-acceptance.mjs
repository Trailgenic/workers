...
assert(hikingModel.summary_statistics?.hiking_sessions === 40, "hiking world model should report 40 hiking sessions");
assert(hikingModel.summary_statistics?.total_distance_miles === 430.66, "hiking world model should report 430.66 miles");
assert(hikingModel.summary_statistics?.total_elevation_gain_ft === 164610, "hiking world model should report 164610 ft gain");
assert(hikingModel.movement_architecture?.total_public_structured_sessions === 103, "movement architecture should report 103 structured sessions");
...
assert(hikingAlias.response.ok && hikingAlias.json.dataset_id === "tg_hikeworldmodel_v3_1", "/datasets/hiking/world-model should return tg_hikeworldmodel_v3_1");
...
for (const check of conditioningChecks) {
  ...
  assert(content.privacy_scope?.data_granularity === "aggregate_plus_selected_sessions", `${check.name} should expose aggregates plus selected sessions`);
...
for (const datasetId of ["tg_walking_conditioning_v1", "tg_rucking_conditioning_v1", "tg_running_conditioning_v1", "tg_hikeworldmodel_v3_1"]) {
